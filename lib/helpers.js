...
function buildNav(user) {
  ...
  return `
    <style>
      ...
      .mobile-menu{display:none;position:fixed;inset:0;background:rgba(15,12,41,0.98);z-index:150;flex-direction:column;padding:20px;overflow-y:auto}
    ...
    <div id="mobile-nav" class="mobile-menu">
      <div class="mobile-menu-header">
        ...
        <button class="mobile-menu-close" onclick="document.getElementById('mobile-nav').classList.remove('show')">&times;</button>
      </div>
      <div class="mobile-user-info">
        <img src="${escapeHtml(user.custom_photo || user.photo || '')}" alt="">
        <div>
          <div class="mobile-user-name">${escapeHtml(user.nickname || user.name)}</div>
          <div class="mobile-user-points">${(user.points || 0).toLocaleString()}P</div>
        </div>
      </div>
      <div class="mobile-menu-links">
        <div class="mobile-menu-section">금융</div>
        <a href="/dashboard" onclick="document.getElementById('mobile-nav').classList.remove('show')">📊 금융정보</a>
        <a href="/dart" onclick="document.getElementById('mobile-nav').classList.remove('show')">📢 주요공시</a>
        <div class="mobile-menu-section">커뮤니티</div>
        <a href="/fear-index" onclick="document.getElementById('mobile-nav').classList.remove('show')">🗳️ 종목 민심</a>
        <a href="/community" onclick="document.getElementById('mobile-nav').classList.remove('show')">💬 생각나누기</a>
        <a href="/contest" onclick="document.getElementById('mobile-nav').classList.remove('show')">🏆 치킨배</a>
        <div class="mobile-menu-section">스터디</div>
        <a href="/study" onclick="document.getElementById('mobile-nav').classList.remove('show')">📚 스터디방</a>
        <div class="mobile-menu-section">리포트</div>
        <a href="/report-dashboard" onclick="document.getElementById('mobile-nav').classList.remove('show')">🏠 리포트 대시보드</a>
        <a href="/reports" onclick="document.getElementById('mobile-nav').classList.remove('show')">🔍 리포트 검색</a>
        <a href="/author/reports/new" onclick="document.getElementById('mobile-nav').classList.remove('show')">✏️ 리포트 작성하기</a>
        <a href="/visit-notes" onclick="document.getElementById('mobile-nav').classList.remove('show')">🗒️ 탐방노트</a>
        <div class="mobile-menu-section">내 활동</div>
        <a href="/my/profile" onclick="document.getElementById('mobile-nav').classList.remove('show')">👤 내 프로필</a>
        <a href="/author/dashboard" onclick="document.getElementById('mobile-nav').classList.remove('show')">📊 내 리포트</a>
        <a href="/my/purchases" onclick="document.getElementById('mobile-nav').classList.remove('show')">📦 내 구매내역</a>
        <a href="/my/points" onclick="document.getElementById('mobile-nav').classList.remove('show')">💰 포인트</a>
        <a href="/my/shop" onclick="document.getElementById('mobile-nav').classList.remove('show')">🛒 상점</a>
        ${user.role === 'admin' ? '<div class="mobile-menu-section">관리자</div><a href="/admin">⚙️ 관리자 대시보드</a>' : ''}
        <div style="padding:10px 18px;margin-top:12px;border-top:1px solid rgba(255,255,255,0.06)">
          <a href="#" onclick="document.getElementById('mobile-nav').classList.remove('show');document.getElementById('contact-modal').classList.add('show');return false" style="display:block;padding:12px;background:rgba(251,191,36,0.1);border:1px solid rgba(251,191,36,0.2);border-radius:12px;text-align:center;color:#fbbf24;font-weight:700;text-decoration:none;font-size:0.9rem">💬 관리자 문의</a>
        </div>
        <div style="padding:10px 18px">
          <a href="/logout" style="display:block;padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;text-align:center;color:rgba(255,255,255,0.5);text-decoration:none;font-size:0.85rem">로그아웃</a>
        </div>
      </div>
    </div>
    <script>
