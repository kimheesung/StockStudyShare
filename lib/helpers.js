...
function buildNav(user) {
  ...
  return `
    <style>
      ...
      .contact-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:1000;align-items:center;justify-content:center;overflow-y:auto;padding:20px}
      .contact-overlay.show{display:flex}
      .contact-modal{background:linear-gradient(135deg,#1a1a3e,#2a2a5e);border:1px solid rgba(255,255,255,0.1);border-radius:24px;padding:32px;max-width:440px;width:92%;margin:auto;animation:contactIn 0.2s ease-out}
    ...
    <div class="contact-overlay" id="contact-modal">
      <div class="contact-modal">
        ...
        <select id="cm-category">
          <option value="feature">기능 요청 / 제안</option>
          <option value="bug">버그 / 오류 신고</option>
          <option value="general">일반 문의</option>
          <option value="other">기타</option>
        </select>
        <textarea id="cm-text" placeholder="자유롭게 작성해주세요. 스크린샷이 있으면 상황 설명과 함께 보내주세요."></textarea>
        <div class="cm-actions">
          <button class="cm-btn-send" id="cm-send" onclick="sendContactMsg()">보내기</button>
          <button class="cm-btn-cancel" onclick="document.getElementById('contact-modal').classList.remove('show')">닫기</button>
        </div>
      </div>
    </div>
    <script>
      document.getElementById('contact-modal').addEventListener('click',function(e){if(e.target===this)this.classList.remove('show')});
