...
const FAVICON_TAG = `<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Cdefs%3E%3ClinearGradient id='g' x1='0' y1='0' x2='1' y2='1'%3E%3Cstop offset='0%25' stop-color='%234f46e5'/%3E%3Cstop offset='100%25' stop-color='%2306b6d4'/%3E%3C/linearGradient%3E%3C/defs%3E%3Crect width='32' height='32' rx='8' fill='url(%23g)'/%3E%3Ctext x='16' y='23' text-anchor='middle' font-family='Arial,sans-serif' font-weight='900' font-size='20' fill='white'%3ES%3C/text%3E%3C/svg%3E">`;
const ADSENSE_TAG = `<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-1724162315254624" crossorigin="anonymous"><\/script>`;
...
function render(filePath, replacements = {}) {
  ...
  // favicon + AdSense 자동 삽입
  if (html.includes('</head>')) {
    let inject = '';
    if (!html.includes('rel="icon"')) inject += FAVICON_TAG;
    if (!html.includes('adsbygoogle')) inject += ADSENSE_TAG;
...
function buildNav(user) {
  ...
  return `
    <style>
      ...
      /* 햄버거 메뉴 (모바일) */
      .nav-hamburger{display:none;background:none;border:none;color:#fff;font-size:1.5rem;cursor:pointer;padding:4px 8px;line-height:1}
      .mobile-menu{display:none;position:fixed;top:0;left:0;right:0;bottom:0;width:100%;height:100%;background:rgba(15,12,41,0.99);z-index:9999;flex-direction:column;padding:20px;overflow-y:auto}
      .mobile-menu.show{display:flex}
      .mobile-menu-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,0.08)}
      .mobile-menu-close{background:none;border:none;color:rgba(255,255,255,0.5);font-size:1.6rem;cursor:pointer}
      .mobile-menu-links{display:flex;flex-direction:column;gap:4px}
      .mobile-menu-links a{display:block;padding:14px 18px;color:rgba(255,255,255,0.8);text-decoration:none;font-size:0.95rem;font-weight:600;border-radius:12px;transition:background 0.15s}
      .mobile-menu-links a:hover,.mobile-menu-links a:active{background:rgba(79,70,229,0.15)}
      .mobile-menu-section{font-size:0.72rem;color:rgba(255,255,255,0.3);text-transform:uppercase;font-weight:700;padding:16px 18px 6px;letter-spacing:0.5px}
      .mobile-user-info{display:flex;align-items:center;gap:12px;padding:16px 18px;margin-bottom:8px;background:rgba(255,255,255,0.03);border-radius:14px}
      .mobile-user-info img{width:40px;height:40px;border-radius:50%;border:2px solid rgba(79,70,229,0.5)}
      .mobile-user-name{font-weight:700;font-size:0.95rem}
      .mobile-user-points{font-size:0.78rem;color:#fbbf24;font-weight:700;margin-top:2px}
      @media(max-width:768px){
        nav{padding:10px 16px}
        .nav-links,.user-area>a,.user-area>span,.user-area .logout-btn,.btn-contact{display:none!important}
        .nav-hamburger{display:block}
        .user-area{gap:8px}
      }
      .contact-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:1000;align-items:flex-start;justify-content:center;overflow-y:auto;padding:40px 20px}
      .contact-overlay.show{display:flex}
      .contact-modal{background:linear-gradient(135deg,#1a1a3e,#2a2a5e);border:1px solid rgba(255,255,255,0.1);border-radius:24px;padding:32px;max-width:440px;width:92%;margin:auto;flex-shrink:0;animation:contactIn 0.2s ease-out}
    ...
    <div class="nav-links">
      <a href="/dashboard" class="nav-item">📊 금융정보</a>
      <a href="/study" class="nav-item">📚 스터디방</a>
      <div class="nav-dropdown">
        <span class="nav-item">📄 리포트 ▾</span>
        <div class="nav-dropdown-menu">
          <a href="/report-dashboard">🏠 리포트 대시보드</a>
    ...
    <div class="user-area">
      <button class="nav-hamburger" onclick="document.getElementById('mobile-nav').classList.add('show')">☰</button>
    ...
    <script>
    // 모바일 메뉴를 body 직속에 생성 (nav의 backdrop-filter stacking context 회피)
    (function(){
      var mn=document.createElement('div');mn.id='mobile-nav';mn.className='mobile-menu';
      mn.innerHTML='<div class="mobile-menu-header"><a href="/" class="logo">StockStudyShare<\\/a><button class="mobile-menu-close" onclick="document.getElementById(\\'mobile-nav\\').classList.remove(\\'show\\')">&times;<\\/button><\\/div>'
        +'<div class="mobile-user-info"><img src="${escapeHtml(user.custom_photo || user.photo || '')}" alt=""><div><div class="mobile-user-name">${escapeHtml(user.nickname || user.name)}<\\/div><div class="mobile-user-points">${(user.points || 0).toLocaleString()}P<\\/div><\\/div><\\/div>'
        +'<div class="mobile-menu-links">'
        +'<div class="mobile-menu-section">금융<\\/div><a href="/dashboard">📊 금융정보<\\/a><a href="/dart">📢 주요공시<\\/a>'
        +'<div class="mobile-menu-section">커뮤니티<\\/div><a href="/fear-index">🗳️ 종목 민심<\\/a><a href="/community">💬 생각나누기<\\/a><a href="/contest">🏆 치킨배<\\/a>'
        +'<div class="mobile-menu-section">스터디<\\/div><a href="/study">📚 스터디방<\\/a>'
        +'<div class="mobile-menu-section">리포트<\\/div><a href="/report-dashboard">🏠 리포트 대시보드<\\/a><a href="/reports">🔍 리포트 검색<\\/a><a href="/author/reports/new">✏️ 리포트 작성하기<\\/a><a href="/visit-notes">🗒️ 탐방노트<\\/a>'
        +'<div class="mobile-menu-section">내 활동<\\/div><a href="/my/profile">👤 내 프로필<\\/a><a href="/author/dashboard">📊 내 리포트<\\/a><a href="/my/purchases">📦 내 구매내역<\\/a><a href="/my/points">💰 포인트<\\/a><a href="/my/shop">🛒 상점<\\/a>'
        +'${user.role === 'admin' ? '<div class="mobile-menu-section">관리자<\\/div><a href="/admin">⚙️ 관리자 대시보드<\\/a>' : ''}'
        +'<div style="padding:10px 18px;margin-top:12px;border-top:1px solid rgba(255,255,255,0.06)"><a href="#" onclick="document.getElementById(\\'mobile-nav\\').classList.remove(\\'show\\');document.getElementById(\\'contact-modal\\').classList.add(\\'show\\');return false" style="display:block;padding:12px;background:rgba(251,191,36,0.1);border:1px solid rgba(251,191,36,0.2);border-radius:12px;text-align:center;color:#fbbf24;font-weight:700;text-decoration:none;font-size:0.9rem">💬 관리자 문의<\\/a><\\/div>'
        +'<div style="padding:10px 18px"><a href="/logout" style="display:block;padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;text-align:center;color:rgba(255,255,255,0.5);text-decoration:none;font-size:0.85rem">로그아웃<\\/a><\\/div>'
        +'<\\/div>';
    ...
    (function(){
      ...
      // 광고 배너 표시 여부 체크 - 숨김 시 하드코딩 배너 모두 제거
      fetch('/api/show-ad-banner').then(function(r){return r.json()}).then(function(d){
        if(!d.show){
          document.querySelectorAll('a[href*="ad-inquiry"]').forEach(function(el){
            if(!el.closest('.admin-link'))el.style.display='none';
          });
          document.querySelectorAll('.sss-ad-slot').forEach(function(el){el.style.display='none'});
        }
      }).catch(function(){});
    })();
    </script>
    <script>
      // 모달을 body 직속에 생성 (nav의 backdrop-filter stacking context 회피)
      (function(){
        var m=document.createElement('div');
        m.className='contact-overlay';m.id='contact-modal';
        m.innerHTML='<div class="contact-modal">'
          +'<span class="cm-beta">BETA<\/span>'
          +'<h3>관리자에게 문의하기<\/h3>'
          +'<p class="cm-desc">현재 베타 단계로 운영 중입니다.<br>기능 요청, 버그/오류 신고, 기타 문의를 편하게 보내주세요!<\/p>'
          +'<div class="cm-msg" id="cm-msg"><\/div>'
          +'<select id="cm-category"><option value="feature">기능 요청 / 제안<\/option><option value="bug">버그 / 오류 신고<\/option><option value="general">일반 문의<\/option><option value="other">기타<\/option><\/select>'
          +'<textarea id="cm-text" placeholder="자유롭게 작성해주세요."><\/textarea>'
          +'<div class="cm-actions"><button class="cm-btn-send" id="cm-send">보내기<\/button><button class="cm-btn-cancel" id="cm-close">닫기<\/button><\/div>'
          +'<\/div>';
        document.body.appendChild(m);
        m.addEventListener('click',function(e){if(e.target===m)m.classList.remove('show');});
        document.getElementById('cm-close').addEventListener('click',function(){m.classList.remove('show');});
        document.getElementById('cm-send').addEventListener('click',function(){sendContactMsg();});
      })();
      function sendContactMsg(){
        var btn=document.getElementById('cm-send'),msg=document.getElementById('cm-msg'),txt=document.getElementById('cm-text').value.trim();
        if(!txt){msg.textContent='내용을 입력해주세요.';msg.className='cm-msg err';return;}
        btn.disabled=true;btn.textContent='전송 중...';msg.className='cm-msg';
        fetch('/api/admin-message',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',
          body:JSON.stringify({category:document.getElementById('cm-category').value,message:txt})
        }).then(function(r){return r.json()}).then(function(d){
          btn.disabled=false;btn.textContent='보내기';
          if(d.ok){msg.textContent='전송 완료! 빠른 시일 내에 확인하겠습니다.';msg.className='cm-msg ok';document.getElementById('cm-text').value='';}
          else{msg.textContent=d.error||'전송 실패';msg.className='cm-msg err';}
        }).catch(function(){btn.disabled=false;btn.textContent='보내기';msg.textContent='네트워크 오류';msg.className='cm-msg err';});
      }
    </script>`;
}
